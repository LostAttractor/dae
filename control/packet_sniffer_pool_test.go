/*
*  SPDX-License-Identifier: AGPL-3.0-only
*  Copyright (c) 2022-2025, daeuniverse Organization <dae@v2raya.org>
 */

package control

import (
	"encoding/hex"
	"net/netip"
	"sync"
	"testing"
	"time"

	"github.com/daeuniverse/dae/component/sniffing"
)

func TestPacketSnifferPoolStaleRemoveKeepsReplacement(t *testing.T) {
	p := NewPacketSnifferPool()
	key := PacketSnifferKey{
		LAddr: netip.MustParseAddrPort("192.0.2.1:1234"),
		RAddr: netip.MustParseAddrPort("192.0.2.2:443"),
	}
	old, _ := p.GetOrCreate(key, &PacketSnifferOptions{Ttl: time.Hour})
	p.pool.Delete(key)
	replacement, _ := p.GetOrCreate(key, &PacketSnifferOptions{Ttl: time.Hour})
	if err := p.Remove(key, old); err != nil {
		t.Fatal(err)
	}
	if got := p.Get(key); got != replacement {
		t.Fatalf("stale removal removed replacement: got %p, want %p", got, replacement)
	}

	old.deadlineTimer.Stop()
	old.Close()
	if err := p.Remove(key, replacement); err != nil {
		t.Fatal(err)
	}
}

func TestPacketSnifferPoolExpiryWaitsForActiveSniffer(t *testing.T) {
	p := NewPacketSnifferPool()
	key := PacketSnifferKey{
		LAddr: netip.MustParseAddrPort("192.0.2.1:1234"),
		RAddr: netip.MustParseAddrPort("192.0.2.2:443"),
	}
	sniffer, _ := p.GetOrCreate(key, &PacketSnifferOptions{Ttl: 20 * time.Millisecond})
	sniffer.Mu.Lock()
	time.Sleep(50 * time.Millisecond)
	if got := p.Get(key); got != sniffer {
		t.Fatal("expiry removed a sniffer while it was active")
	}
	sniffer.Mu.Unlock()

	deadline := time.After(time.Second)
	for p.Get(key) != nil {
		select {
		case <-deadline:
			t.Fatal("expired sniffer was not removed after becoming idle")
		default:
			time.Sleep(time.Millisecond)
		}
	}
}

func TestPacketSnifferLimitRemovesSessionBeforeQueuedAppends(t *testing.T) {
	p := NewPacketSnifferPool()
	key := PacketSnifferKey{
		LAddr: netip.MustParseAddrPort("192.0.2.1:1234"),
		RAddr: netip.MustParseAddrPort("192.0.2.2:443"),
	}
	sniffer, _ := p.GetOrCreate(key, &PacketSnifferOptions{Ttl: time.Hour})

	const workers = 64
	start := make(chan struct{})
	var ready, done sync.WaitGroup
	appended := 0
	ready.Add(workers)
	done.Add(workers)
	for range workers {
		go func() {
			defer done.Done()
			ready.Done()
			<-start

			sniffer.Mu.Lock()
			if p.Get(key) == sniffer {
				sniffer.AppendData([]byte{0})
				appended++
				if appended == 33 {
					_ = p.removeLocked(key, sniffer)
				}
			}
			sniffer.Mu.Unlock()
		}()
	}
	ready.Wait()
	close(start)
	done.Wait()

	if got := p.Get(key); got != nil {
		t.Fatalf("limited sniffer remains in pool: %p", got)
	}
	// One sentinel plus the 32-packet allowance and its triggering datagram.
	if got, want := len(sniffer.Data()), 34; got != want {
		t.Fatalf("retained slices = %d, want %d", got, want)
	}
}

var testPacketSnifferData = []string{
	"",
	"",
}

func TestPacketSnifferOldTimerCannotRemoveReplacement(t *testing.T) {
	pool := NewPacketSnifferPool()
	key := PacketSnifferKey{
		LAddr: netip.MustParseAddrPort("1.1.1.1:1000"),
		RAddr: netip.MustParseAddrPort("2.2.2.2:2000"),
	}
	old, _ := pool.GetOrCreate(key, &PacketSnifferOptions{Ttl: 20 * time.Millisecond})
	if err := pool.Remove(key, old); err != nil {
		t.Fatal(err)
	}
	replacement, _ := pool.GetOrCreate(key, &PacketSnifferOptions{Ttl: time.Second})
	t.Cleanup(func() { _ = pool.Remove(key, replacement) })
	time.Sleep(50 * time.Millisecond)
	if got := pool.Get(key); got != replacement {
		t.Fatalf("replacement sniffer = %p, want %p", got, replacement)
	}
}

func TestPacketSniffer_Normal(t *testing.T) {
	for _, _data := range testPacketSnifferData {
		data, _ := hex.DecodeString(_data)
		sniffer, _ := DefaultPacketSnifferSessionMgr.GetOrCreate(PacketSnifferKey{
			LAddr: netip.MustParseAddrPort("1.1.1.1:1111"),
			RAddr: netip.MustParseAddrPort("2.2.2.2:2222"),
		}, nil)
		sniffer.AppendData(data)
		domain, err := sniffer.SniffUdp()
		if err != nil && !sniffing.IsSniffingError(err) {
			t.Fatal(err)
		}
		if sniffer.NeedMore() {
			continue
		}
		sniffer.Close()
		t.Log(domain)
		return
	}
	t.Fatal("not found")
}

func TestPacketSniffer_Mismatched(t *testing.T) {
	dst := netip.MustParseAddrPort("2.2.2.2:2222")
	for _, _data := range testPacketSnifferData {
		data, _ := hex.DecodeString(_data)
		sniffer, _ := DefaultPacketSnifferSessionMgr.GetOrCreate(PacketSnifferKey{
			// Use a key different from TestPacketSniffer_Normal: sessions
			// stay in the pool after Close and would otherwise be reused.
			LAddr: netip.MustParseAddrPort("3.3.3.3:3333"),
			RAddr: dst,
		}, nil)
		sniffer.AppendData(data)
		domain, err := sniffer.SniffUdp()
		if err != nil && !sniffing.IsSniffingError(err) {
			t.Fatal(err)
		}
		if sniffer.NeedMore() {
			dst = netip.AddrPortFrom(dst.Addr(), dst.Port()+1)
			continue
		}
		sniffer.Close()
		t.Fatal("unexpected found", domain)
		return
	}
}
