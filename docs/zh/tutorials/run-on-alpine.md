...
**注意：** 
1. dae 要求 Linux 6.13 或更新版本并启用 `CONFIG_NETKIT=y`；请检查实际内核，而不是仅检查 Alpine 版本。
2. 从 3.20 版开始，由于 Alpine Linux 的跨 CPU 架构兼容性问题，官方禁用了 dae 所需的一些功能，因此只能使用配置满足要求的 `linux-virt` 内核。对于 `linux-lts` 或 `linux-edge`，你可能需要自己构建内核。
3. 本教程适用于 Alpine Linux 3.20 及更新版本，且内核必须满足上述要求。
